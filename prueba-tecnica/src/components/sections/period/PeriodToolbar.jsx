...
let PeriodToolbar = props => {
  const { numSelected, classes, onEditClick, onDeleteClick, onAddClick, onPdf, onReturnFiles, periodos, handleExportClose, handleExportClick, anchorEl } = props
  var anchor = null
  var multiDataSet = []
  if(periodos.periodos.length!=0){
    multiDataSet = [
      {
          columns:["Número Período", "Estado", "Descripción", "Día Inicial", "Duración Días"],
          data: []
      }
    ]
    for (var i = 0; i < periodos.periodos.length; i++) {
      multiDataSet[0].data.push([periodos.periodos[i].nroPeriodo,
        periodos.periodos[i].estado,
        periodos.periodos[i].descripcion,
        periodos.periodos[i].diaInicio,
        periodos.periodos[i].duracionDias])
    }
  }
      ...
      <Grid container spacing={0}>
        <Grid item xs={6}>
          ...
          <ExcelFile  element={(
            ...
            <ExcelSheet dataSet={multiDataSet} name="Periodos" />
          </ExcelFile>
      ...
      <div className={classes.title}>
        {numSelected > 0 ? (
          <Typography type="subheading">{numSelected} selected</Typography>
            ...
            <Typography type="title">Periodos</Typography>
          )}
      </div>
...
PeriodToolbar = withStyles(toolbarStyles)(PeriodToolbar)
