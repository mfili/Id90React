...
let FilesToolbar = props => {
  const { numSelected, classes, onEditClick, onDeleteClick, onAddClick, onPdf, legajos, handleExportClose, handleExportClick, anchorEl } = props
  var anchor = null
  var multiDataSet = []
  if(legajos.legajos.length!=0){
    multiDataSet = [
      {
          columns:["Legajo", "Cuil", "Apellido", "Nombre"],
          data: []
      }
    ]
    for (var i = 0; i < legajos.legajos.length; i++) {
      multiDataSet[0].data.push([legajos.legajos[i].legajo,legajos.legajos[i].cuil,legajos.legajos[i].apellido,legajos.legajos[i].nombre])
    }
  }
      ...
      <Grid container spacing={0}>
        <Grid item xs={6}>
          ...
          <ExcelFile  element={(
            ...
            <ExcelSheet dataSet={multiDataSet} name="Legajos" />
          </ExcelFile>
      ...
      <div className={classes.title}>
        {numSelected > 0 ? (
          <Typography type="subheading">{numSelected} selected</Typography>
            ...
            <Typography type="title">Legajos</Typography>
          )}
      </div>
...
FilesToolbar = withStyles(toolbarStyles)(FilesToolbar)
