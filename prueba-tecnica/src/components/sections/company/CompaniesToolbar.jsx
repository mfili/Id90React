...
let CompaniesToolbar = props => {
  const { numSelected, classes, onEditClick, onDeleteClick, onAddClick, onPdf, companies, handleExportClose, handleExportClick, anchorEl } = props
  var anchor = null
  var multiDataSet = []
  if(companies.companies.length!=0){
    multiDataSet = [
      {
          columns:["Código", "Razon Social", "Cuit"],
          data: []
      }
    ]
    for (var i = 0; i < companies.companies.length; i++) {
      multiDataSet[0].data.push([companies.companies[i].companyId,companies.companies[i].razonSocial,companies.companies[i].cuit])
    }
  }
      ...
      <Grid container spacing={0}>
        <Grid item xs={6}>
          ...
          <ExcelFile  element={(
            ...
            <ExcelSheet dataSet={multiDataSet} name="Empresas" />
          </ExcelFile>
      ...
      <div className={classes.title}>
        {numSelected > 0 ? (
          <Typography type="subheading">{numSelected} selected</Typography>
            ...
            <Typography type="title">Empresas</Typography>
          )}
      </div>
...
CompaniesToolbar = withStyles(toolbarStyles)(CompaniesToolbar)
