...
let ClassToolbar = props => {
  const { numSelected, classes, onEditClick, listItems, onDeleteClick, onAddClick, onPdf, clases, handleExportClose, handleExportClick, anchorEl } = props
  var anchor = null
  var multiDataSet = []
  if(clases.clase.length!=0){
    multiDataSet = [
      {
    ...
    for (var i = 0; i < clases.clase.length; i++) {
      multiDataSet[0].data.push([clases.clase[i].idClase,clases.clase[i].descripcion])
    }
  }
  ...
  if (numSelected == 1) {
    iconButon = <div>
      <Grid container spacing={0}>
        ...
        <Grid item xs={3}>
          <Tooltip title="Items">
            <IconButton aria-label="Items" onClick={listItems}>
              <List />
            </IconButton>
          </Tooltip>
        </Grid>
      ...
      <div className={classes.title}>
        {numSelected > 0 ? (
          <Typography type="subheading">{numSelected} selected</Typography>
            ...
            <Typography type="title">Clases</Typography>
          )}
      </div>
...
ClassToolbar = withStyles(toolbarStyles)(ClassToolbar)
