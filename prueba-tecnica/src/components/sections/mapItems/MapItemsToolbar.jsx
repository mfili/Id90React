...
let MapItemsToolbar = props => {
  const { numSelected, classes, onEditClick, onDeleteClick, onAddClick, items, onPdf, onReturnFiles, handleExportClose, handleExportClick, anchorEl } = props
  var anchor = null
  var multiDataSet = []
  /*if(items.length!=0){
    multiDataSet = [
      {
          columns:["Código", "Descripción", "Período"],
          data: []
      }
    ]
    for (var i = 0; i < items.length; i++) {
      multiDataSet[0].data.push([items[i].items.codigoConcepto,
                  items[i].items.descripcionConcepto,
                  items[i].periodo.descripcion])
    }
  }*/
      ...
      <Grid container spacing={0}>
        ...
        <Grid item xs={4}>
          ...
          <ExcelFile  element={(
            ...
            <ExcelSheet dataSet={multiDataSet} name="MapaItems" />
          </ExcelFile>
      ...
      <div className={classes.title}>
        {numSelected > 0 ? (
          <Typography type="subheading">{numSelected} selected</Typography>
            ...
            <Typography type="title">Mapa de Items</Typography>
          )}
      </div>
...
MapItemsToolbar.propTypes = {
  classes: PropTypes.object.isRequired,
  numSelected: PropTypes.number.isRequired
}

MapItemsToolbar = withStyles(toolbarStyles)(MapItemsToolbar)
export default MapItemsToolbar